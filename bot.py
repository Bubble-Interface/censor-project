...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Please send me a photo to censor",
    )
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    original_image_path = os.path.join(user_original_images_dir, f"{photo_uuid}.jpg")
    ...
    await update.message.reply_text(
        "Now, please send me text that needs to be censored on the photo."
    )
...
async def censor_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    original_image = cv2.imread(context.user_data["original_image_path"])
    reader = easyocr.Reader(['en'], gpu=False)
    result = reader.readtext(context.user_data["original_image_path"], width_ths=0)
    full_processed_image_file_name = ''
    for detection in result:
        if detection[1] in censor_text:
            top_left = tuple([int(val) for val in detection[0][0]])
            bottom_right = tuple([int(val) for val in detection[0][2]])
            processed_image = cv2.rectangle(original_image, top_left, bottom_right, (0,255,0),5)
